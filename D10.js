...
console.log(me);
//Messaggio per Stefano: non tenere conto delle skills le sto imparando adesso, non sapevo cosa inserire
/* ESERCIZIO G
...
//altrimenti potevo usare il metodo Math.max(num1,num2), ottenendo lo stesso risultato
...
const splitMe = (string) => {
  ...
  for (let i = 0; i < word.length; i++) {
    let finalWord = word[i].charAt(0).toUpperCase() + word[i].slice(1);
    finalSting.push(finalWord);
  }

  return finalSting.join(" ");
};
...
/* ESERCIZIO 4
  ...
  Se il valore booleano è true la funzione deve ritornare la stringa senza il primo carattere, altrimenti la deve ritornare
   senza l'ultimo.
*/
const deleteOne = (string, bValue) => {
  let oneCharacter = 0;
  if (bValue) {
    oneCharacter = string.charAt(0);
  } else {
    oneCharacter = string.slice(-1);
  }
  return oneCharacter;
};
...
const isThisAnEmail = (email) => {
  if (email.includes("@") && email.includes(".")) {
    console.log("Questo è un indirizzo e-mail valido");
  } else {
    console.log("Questo non è un indirizzo e-mail valido");
  }
};
isThisAnEmail("marylucentini@gmail.com");
...
const rollTheDices = (num) => {
  ...
  for (let i = 0; i < num; i++) {
    objreturn.values.push(dice());
  }
  for (let i = 0; i < objreturn.values.length; i++) {
    objreturn.sum = objreturn.sum + objreturn.values[i];
  }
  return objreturn;
};
...
// */
// const howManyDays = (year, month, day) => {
//   let now = new Date().valueOf();
//   let dataFornita = new Date(year, month, day).valueOf();
//   let howMany = dataFornita - now;
//   // let howManyDays = today - data;
//   return howMany;
// };
// console.log(howManyDays(2024, 3, 13));

// // non sono riuscita a farlo mi dispiace
/* ESERCIZIO 10
...
isTodayMyBirthday(10, 5);
isTodayMyBirthday(15, 2);
//l'ho avviata 2 volte per verificare se funzionasse correttamente
...
console.log(deleteProp(person, "color-eyes"));
/* ESERCIZIO 12
...
const newestMovie = () => {
  ...
  for (let i = 0; i < movies.length; i++) {
    newMovieYear = Math.max(parseInt(movies[i].Year));
    if (parseInt(movies[i].Year) === newMovieYear) {
      newMovie = movies[i];
    }
  }
...
const onlyInLastMillennium = () => {
  ...
  for (let i = 0; i < movies.length; i++) {
    if (parseInt(movies[i].Year) >= 2000) {
      onlyInThisMillennium.push(movies[i]);
    }
  }
...
//avrei potuto utilizzare reduce, ma ancora non ho molta dimestichezza con questi metodi, quindi ho preferito fare la versione estesa. Questo fine settimana mi eserciterò
/* ESERCIZIO 17
...
const searchAndDivide = (string) => {
  ...
  for (let i = 0; i < movies.length; i++) {
    if (movies[i].Title.includes(string)) {
      searchAndDivideObj.match.push(movies[i]);
    } else {
      searchAndDivideObj.unmatch.push(movies[i]);
    }
  }
  return searchAndDivideObj;
};
